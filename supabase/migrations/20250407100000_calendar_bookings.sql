...
-- ============================================
-- 0. PREIMENUJ STARU BOOKINGS TABLICU AKO POSTOJI
-- ============================================
DO $$
BEGIN
    -- Ako postoji bookings tablica bez provider_id stupca, to je stara tablica
    IF EXISTS (
        ...
        -- Preimenuj staru tablicu
        ALTER TABLE public.bookings RENAME TO sitter_bookings_old;
        RAISE NOTICE 'Stara bookings tablica preimenovana u sitter_bookings_old';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.ical_feeds (
  ...
  exclude_services TEXT[] DEFAULT ARRAY[]::TEXT[],
...
-- ============================================
-- 99. UPDATE PAYMENTS FOREIGN KEY
-- ============================================
-- After renaming old bookings to sitter_bookings_old, update payments FK
DO $$
BEGIN
    -- Check if payments table exists and has FK to bookings
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        ...
        -- Add new FK to sitter_bookings_old
        ALTER TABLE public.payments 
        ADD CONSTRAINT payments_booking_id_fkey 
        FOREIGN KEY (booking_id) REFERENCES public.sitter_bookings_old(id);
        
        RAISE NOTICE 'Updated payments FK to reference sitter_bookings_old';
    END IF;
END $$;
