...
export async function getGroomers(filters?: GroomerFilters): Promise<Groomer[]> {
  ...
  try {
    ...
    let query = supabase.from('groomers').select('id, name, city, services, prices, rating, review_count, bio, verified, specialization, user_id, phone, email, address, working_hours');
...
export async function getGroomer(id: string): Promise<Groomer | null> {
  ...
  try {
    const supabase = await createClient();
    const { data, error } = await supabase.from('groomers').select('id, name, city, services, prices, rating, review_count, bio, verified, specialization, user_id, phone, email, address, working_hours').eq('id', id).single();
    if (!error && data) return data as Groomer;

    // Legacy fallback: some old links may use numeric indexes (1,2,3...) instead of UUIDs.
    if (/^\d+$/.test(id)) {
      const { data: list } = await supabase.from('groomers').select('id, name, city, services, prices, rating, review_count, bio, verified, specialization, user_id, phone, email, address, working_hours').order('name');
